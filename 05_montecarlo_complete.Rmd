...
2. Run the model for one year of the study period and perform a GLUE-based sensitivity analysis.
...
nx <- 100 # number of runs (parameter tests)

# set up an empty parameter matrix for the loss function, transfer function, and objective functions
param <- tibble(
...
# there are still no values in param, the MC code will fill this tibble as we go. 
```
...
for (ii in 1:nx) {
  ...
  while (sum(p_eff) < 0.2 * sum(Pobs) ||
    ...
    # Here, you need to specify how to generate the values for the loss function parameters. Use runif() for the sampling. For each loop iteration, we want to create one value for each parameter (and save it in param tibble)
    ...
    # preallocate the empty p_eff vector (effective precipitation) (length of the timeseries)
    p_eff <- vector(mode = "double", length(tau))

    # set a starting value for s (you are pulling the value out of the param tibble)
    s <- param$b3[ii]
...
```{r model_output, echo=F, eval=F}
## plot the runs from the calibration to make sure it works properly
# this line does a few things: it takes the Q simulations that are stored in param$qsim as a list and "unlists" them into a single vector (unlist(param$qsim)); then it takes that vector and writes it into a matrix with as many columns as nx; then it takes the matrix and converts it to a dataframe. 
...
```{r model_plt, echo=F, eval=F}
# plot the simulated hydrograph for each simulation
...
**Q2 (3pnt) Understanding the loop: Describe what happens during a single Monte Carlo iteration (one pass through the loop). List the main steps (~6) from parameter sampling to saving model performance. (2–4 sentences)**
ANSWER:


### Save or load data

After setting up the MC simulation, we will actually use a pre-created dataset (like the param matrix generated above). Running the MC simulation tens of thousands of times will take multiple hours. For that reason, we ran it for you and will now use it as an existing data set.
...
**Q3. (3pt) Using the arrange() function (see code after simulation above ~ line 220), rearrange the imported simulation results by any other calculated performance metric (like KGE or MAE...) instead and plot the observed and simulated discharge in the chunk below.** 
...
GLUE is used here because it provides an intuitive and visual way to explore parameter uncertainty and model performance. However, it is not the only approach to sensitivity or uncertainty analysis, and it has known limitations (e.g., subjective behavioral thresholds). In your own modeling work, the most appropriate method will depend on your research question, data availability, and disciplinary context.
For this exercise, we will use NSE as the performance metric to classify behavioral runs, but this can be used for any performance metric. 
...
**Q5 (3 pts) Briefly discuss how parameter sensitivity (or lack of it) influences your confidence in model predictions.**
...
param_ci <- param %>%
  filter(nse > 0.8) # only use obj function (nse) values above the previously defined threshold
...
# combine new dataframe (Qruns) with date and observed runoff from the PQ data frame. additionally, calculate mins and maxs
Qruns <-
...
```{r}
# Lets add observed Q and the 'best' run to the plot
ggplot() +
  geom_ribbon(data = Qruns, aes(x = Date, ymin = Qmin, ymax = Qmax)) + # plot of envelopes. 
